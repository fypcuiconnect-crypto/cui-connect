...
function getTimeGroup(dateString) {
  // Fallback: If no date exists, safely categorize it as "Older"
...
export default function SearchModal({
  ...
  const filteredConversations = useMemo(() => {
    ...
    return conversations.filter((c) => {
      // FIX: Use safe fallbacks to prevent undefined.toLowerCase() crashes
      const safeTitle = c.title || ""
      const safePreview = c.preview || ""
      return safeTitle.toLowerCase().includes(q) || safePreview.toLowerCase().includes(q)
    })
  }, [conversations, query])
  ...
  const groupedConversations = useMemo(() => {
    ...
    // Safely copy the array before sorting, and handle any missing dates safely
    const sortedConvs = [...filteredConversations].sort((a, b) => {
            ...
            <div className="max-h-[60vh] overflow-y-auto">
                    ...
                    <div className="space-y-1">
                      {convs.map((conv) => (
                        <button
                          ...
                          className={`flex w-full items-center gap-3 rounded-lg p-3 text-left hover:bg-zinc-100 dark:hover:bg-zinc-800 ${
                            selectedId === conv.id ? "bg-zinc-50 dark:bg-zinc-800/50" : ""
                          }`}
                        >
              ...
              {filteredConversations.length === 0 && query.trim() && (
                <div className="p-8 text-center">
                  ...
                  <div className="mt-4 text-lg font-medium text-foreground">No chats found</div>
              ...
              {!query.trim() && conversations.length === 0 && (
                <div className="p-8 text-center">
                  <div className="text-lg font-medium text-foreground">No conversations yet</div>
